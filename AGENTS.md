...
5. `measurements/` holds the raw records behind numbers quoted on the pages; `docs/provenance.md` defines the attribution block.
...
8. To add or update a recipe: write `recipe.json` next to the code, copy it into `recipes.json`, run `python tools/validate.py` and `python tools/check_drift.py`.
9. Do not edit text between `<!-- gen:...:start -->` and `<!-- gen:...:end -->` markers; `tools/render.py` regenerates it from `recipes.json`.
